document.addEventListener('DOMContentLoaded', function () {
      ...
      .then(data => {
        ...
        data.forEach((item, index) => {
          ...
          img.addEventListener('click', () => {
            ...
            imageContainer.style.transform = isPhoneViewport() ? 'translateY(50vh)' : 'translateX(50vw)';
            imageContainer.style.width = isPhoneViewport() ? '100vw' : '50vw';
    ...
    closeBtn.addEventListener('click', () => {
      ...
      document.querySelectorAll('.gallery-img').forEach(colImg => {
        colImg.parentElement.style.width = isPhoneViewport() ? '25vw' : '25vw';
      });
  
      setTimeout(() => {
        imageContainer.style.transition = 'transform 1.97s ease, width 1.97s ease';
        imageContainer.style.transform = 'translateX(0)';
        imageContainer.style.width = isPhoneViewport() ? '100vw' : '100vw';
      }, 0);
    });
  });
